...
| `PrefabRef` (instance link) | Instance component that points to the prefab entity (`m_Prefab`) | finding the prefab entity to edit | treating “prefab entity `*Data`” as baseline | `Game.Prefabs.PrefabRef.m_Prefab` *(PrefabRef stores an entity handle)* |
